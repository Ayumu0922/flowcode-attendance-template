      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-8">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-8 space-y-6">
          ...
          <div className="flex gap-4">
            <button onClick={clockIn} disabled={!!record?.clockIn}
              className="flex-1 flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 disabled:bg-zinc-800 disabled:text-zinc-600 text-white py-4 rounded-xl font-semibold transition-colors">
              <LogIn className="w-5 h-5" /> 出勤
            </button>
            <button onClick={clockOut} disabled={!record?.clockIn || !!record?.clockOut}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[{ label: '通常出勤', type: 'normal' }, { label: 'リモート', type: 'remote' }, { label: '有給休暇', type: 'paid-leave' }].map(({ label, type }) => (
            <div key={type} className={`bg-zinc-900/50 border rounded-xl p-3 text-center text-sm ${record?.type === type ? 'border-orange-500/50 text-orange-400' : 'border-zinc-800 text-zinc-500'}`}>{label}</div>
          ))}
        </div>
      </motion.div>
    </div>
